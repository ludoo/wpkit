<?php
/*
Plugin Name: WP Frontman Administrative Settings
Plugin URI: http://wpfrontman.com/
Description: WP Frontman administrative settings
Version: 0.1
...
class WPFrontmanAdmin {
    
    var $db_version = 1;
    var $multi_options_labels = array(
    ...
    var $single_options_labels = array(
        //                                       section name       option label            input   help  default
        'wp_root'                   => array('Options', 'WP installation path', null, null, 'ABSPATH'),
        'cache_destination'         => array('Options', 'Frontend cache endpoint URL', 'text', null, null),
        'preformatter'              => array('Configuration', 'Enable preformatting for posts and pages', 'checkbox', 'Maintain plugins compatibility and speed up the frontend by storing a formatted copy of the content in <code>post_content_filtered</code>', true),
        'support_category_order'    => array('Configuration', 'My Category Order plugin installed', 'checkbox', 'Add support for the category order defined with the <a href="http://wordpress.org/extend/plugins/my-category-order/">My Category Order</a> plugin', false),
        'use_sendfile'              => array('Configuration', 'Use X-Sendfile for static content', 'checkbox', 'Refer to your HTTP/Fastcgi server documentation for details', false),
        );
    var $common_options_labels = array(
        //                                       section name       option label            input   help  default
    ...
    function WPFrontmanAdmin() {
        // initialize defaults
        global $wp_default_secret_key;
        $this->mu = is_multisite();
        if ($this->mu)
            $this->options_labels = array_merge($this->multi_options_labels, $this->common_options_labels);
        else
            $this->options_labels = array_merge($this->single_options_labels, $this->common_options_labels);
        ...
        if ($this->mu)
            ...
            add_action('admin_menu', array(&$this, 'action_network_admin_menu'));
    ...
    function _get_option($opt, $ret=null) {
        if ($this->mu)
            return get_site_option($opt, $ret);
        return get_option($opt, $ret);
    }
    
    function _update_option($opt, $value) {
        if ($this->mu)
            update_site_option('wp_frontman', $value);
        else
            update_option('wp_frontman', $value);
    }
    
    function get_defaults() {
        global $wpdb;
        $defaults = array_map(function ($v) { return $v[4]; }, $this->options_labels);
        $site_url = $this->_get_option('siteurl');
    ...
    function bootstrap($reset=false) {
        if (is_array($this->options))
            return;
        $options = $reset ? array() : $this->_get_option('wp_frontman', array());
        if (!array_key_exists('db_version', $options) || $options['db_version'] != $this->db_version) {
            ...
            $this->_update_option('wp_frontman', $options);
    ...
    function action_network_admin_menu() {
        add_submenu_page(
            ...
            'WP Frontman Admin Settings', 'WP Frontman Admin',
            $this->mu ? 'manage_network_options' : 'manage_options',
            'wp_frontman_admin', array(&$this, 'submenu_page'));
    }
    
    function filter_admin_footer_text($text) {
        return $text . ' &bull; Admin Plugin for <a href="http://wp-frontman.com/">WP Frontman</a> running';
    }
    ...
    function submenu_page() {
        ...
        if ($_POST && !$reset) {
            ...
            if ($update) {
                $this->_update_option('wp_frontman', $this->options);
