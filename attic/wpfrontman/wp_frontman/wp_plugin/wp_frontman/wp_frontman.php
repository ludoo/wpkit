...
class WPFrontman {
    ...
    function init_taxonomies() {
        ...
        foreach ($wp_taxonomies as $k=>$v) {
            ...
            else if (!array_key_exists($k, $this->options['custom_taxonomies']['custom_taxonomies']))
                $this->theme_taxonomies[$k] = $this->_taxonomy_to_array($v);
        }
    }
    ...
    function after_setup_theme() {
        ...
        if ($this->options['custom_taxonomies']['enabled']) {
            foreach ($this->options['custom_taxonomies']['custom_taxonomies'] as $name=>$data) {
                $args = array(
                    'label'         => $data['label'],
                    'public'        => $data['public'],
                    ...
                    'rewrite'       => array(
                        'hierarchical'  => $data['rewrite_hierarchical'],
                        'slug'          => $data['rewite_slug'],
                        'with_front'    => $data['rewrite_with_front']
                    )
                );
                if ($data['update_count'])
                    $args['update_count_callback'] = '_update_post_term_count';
                register_taxonomy($name, $data['object_type'], $args);
            }
        }
    }
    ...
    function options_page() {
        ...
        h2o::addFilter('var_Export', function ($obj) { return var_export($obj, true); });
        h2o::addFilter('pluralize', function ($word, $count, $singular='', $plural='s') {return $word . ($count == 1 ? $singular : $plural);});
        h2o::addFilter('in_array', function ($needle=null, $haystack=null, $text=null) { if (!is_array($haystack)) { return false; } if (in_array($needle, $haystack)) return $text ? $text : true;});
    ...
    function &_options_page_do_custom_taxonomies() {
        ...
        $vars = array(
            ...
            'theme_taxonomies'=>&$this->theme_taxonomies,
            // TODO: add support for custom posts, etc.
            'obj_types'=>array('post', 'link') //array_merge(array_keys(array_filter($wp_post_types, function ($var) { return $var->public;})), array('link'))
        );
        if ($_POST) {
            if (array_key_exists('name', $_POST) && $_POST['name']) {
                // check for a custom taxonomy
                $taxonomy_name = $_POST['name'];
                $deleted = false;
                if (array_key_exists($taxonomy_name, $this->options['custom_taxonomies']['custom_taxonomies'])) {
                    // TODO: don't use the submit button name
                    if ($_POST['submit'] == 'Delete') {
                        $deleted = true;
                        unset($this->options['custom_taxonomies']['custom_taxonomies'][$taxonomy_name]);
                        $this->message[] = "Taxonomy '$taxonomy_name' removed";
                    } else {
                        $this->message[] = "Taxonomy '$taxonomy_name' modified";
                    }
                } else {
                    $this->message[] = "Taxonomy '$taxonomy_name' added";
                }
                if (!$deleted) {
                    $taxonomy = array(
                        'label'=>null, 'rewrite_slug'=>null,
                        'hierarchical'=>null, 'rewrite_hierarchical'=>null,
                        'public'=>null, 'show_ui'=>null, 'update_count'=>null,
                        'object_type'=>array()
                        );
                        ...
                        if (array_key_exists($k, $_POST)) {
                            ...
                            switch ($k) {
                                ...
                                case 'rewrite_slug':
                                    ...
                                    break;
                                default:
                                    $taxonomy[$k] = $value == 'on' ? true : false;
                            }
                        }
                    }
                    $taxonomy['label'] = $taxonomy['label'] ? $taxonomy['label'] : $taxonomy_name;
                    $this->options['custom_taxonomies']['custom_taxonomies'][$taxonomy_name] = $taxonomy;
                }
                update_option('wp_frontman', $this->options);
            } else {
            ...
            //die(var_export(array_keys($vars['obj_type']), true));
        }
        return $vars;
    }
